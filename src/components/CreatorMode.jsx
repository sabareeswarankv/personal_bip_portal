  ...
  const [codingQuestions, setCodingQuestions] = useState(
    editingCourse && editingCourse.type === 'coding'
      ? editingCourse.assessment.questions.map(q => ({
          ...
          sampleCases: (q.sampleTestCases || []).map(c => ({ input: c.input || "", output: c.output || "" })),
          hiddenCases: (q.hiddenTestCases || []).map(c => ({ input: c.input || "", output: c.output || "" }))
        }))
      : [
          {
  ...
  const handleSave = () => {
    ...
    const newCourse = {
      ...
      assessment: {
        ...
        passMarks: courseType === 'mcq' ? 24 : 48,
        negativeMark: 0.25,
        questions: courseType === 'mcq' 
          ...
          : codingQuestions.map((q, idx) => ({
              ...
              hiddenTestCases: q.hiddenCases.map((c, cIdx) => ({ id: cIdx + 3, ...c }))
            }))
      }
    };
                ...
                <div key={qIndex} className="border border-gray-200 p-5 rounded-lg bg-gray-50/50 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-xs font-bold text-gray-500 uppercase">Sample Test Cases (Show on screen)</h4>
                      {q.sampleCases.map((sc, sIdx) => (
                        <div key={sIdx} className="bg-white border border-gray-200 p-3 rounded space-y-2">
                          <span className="text-[10px] font-extrabold text-gray-400">SAMPLE #{sIdx + 1}</span>
                    ...
                    {/* 4 Hidden cases */}
                    <div className="space-y-3">
                      <h4 className="text-xs font-bold text-gray-500 uppercase">Hidden Test Cases (Secret grading)</h4>
