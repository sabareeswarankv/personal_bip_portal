...
export default function App() {
  ...
  const [history, setHistory] = useState(() => {
    const userSaved = localStorage.getItem('personal_ps_assessment_user');
    const parsedUser = userSaved ? JSON.parse(userSaved) : null;
    const key = parsedUser?.email ? `personal_ps_assessment_history_${parsedUser.email}` : 'personal_ps_assessment_history_guest';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "courses"), (snapshot) => {
      ...
      console.error("Firestore courses subscription failed:", error);
    });
  ...
  useEffect(() => {
    const key = user?.email ? `personal_ps_assessment_history_${user.email}` : 'personal_ps_assessment_history_guest';
